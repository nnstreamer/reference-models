...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.scale = 256.0;
    ...
    )
{
    ...
    else
    {
        ...
        {
           printf("wrong jpg file , the jpg file size should be %ld%ld%d\n",
               width, height, depth);
           return VSI_FAILURE;
        }
    ...
    )
{
        ...
        scale != 0)
    {
        ...
        imageprocess->mean.scale = 1.0f / scale;
    ...
    )
{
    ...
    if(fileType == NN_FILE_JPG)
    {
        ...
        _get_jpg_size(image_name, &(attr.size[0]), &(attr.size[1]),
            &(attr.size[2]));
    ...
    {
        printf("vnn_PreProcessYolov3_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
